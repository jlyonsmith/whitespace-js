...
test("test help", async done => {
  ...
  const tool = new _SpacerTool.SpacerTool(mockLog);
...
const space2 = "a:\n" + "{\n" + "\tb: 1,\n" + "  c: 2,\n" + " \t d:\t3\n" + " }\n" + "\n";
const space2_tabs = "a:\n" + "{\n" + "\tb: 1,\n" + "\tc: 2,\n" + "\td:\t3\n" + "}\n" + "\n";
test("space1 info", testGetInfo({
  in: space1,
  info: /, mixed/
}));
test("space2 info", testGetInfo({
  in: space1,
  info: /, mixed/
}));
...
test("space1 to spaces", testConvert({
  in: space1,
  newBol: "spaces",
  out: space1_spaces,
  info: /, mixed.*spaces/
}));
test("space2 to tabs", testConvert({
  in: space2,
  newBol: "tabs",
  round: true,
  out: space2_tabs,
...
//# sourceMappingURL=SpacerTool.test.js.map